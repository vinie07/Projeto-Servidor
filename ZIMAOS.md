...
**Sobre o sistema:** O ZimaOS **não é baseado em Debian** — é construído com **Buildroot** (gerador de sistemas Linux mínimos) rodando kernel **6.12**. Por isso **não tem `apt`/`apt-get`**, o home e o filesystem raiz são **somente-leitura**, e a partição gravável é a **`/DATA`**.
...
**Sobre a ressincronização:** É normal e acontece ao **criar o RAID**, **trocar um disco** ou após um **desligamento inesperado**. O ZimaOS está copiando/espelhando os dados entre os discos.

**RAID não é backup.** Se um arquivo for deletado ou corrompido, o RAID replica o problema em todos os discos. Sempre tenha backups separados.
...
Mostra saúde geral, horas ligado, temperatura e info de RAID.
...
**Atributos que merecem atenção** — se algum dos três primeiros for diferente de 0, fazer **backup urgente**:

| Atributo | O que indica |
|---|---|
| `Reallocated_Sector_Ct` | Setores remapeados — se crescer, o HD está falhando |
| `Current_Pending_Sector` | Setores com erro de leitura pendente |
| `Offline_Uncorrectable` | Setores que não puderam ser corrigidos |
| `Power_On_Hours` | Horas totais de uso do disco |
| `Temperature_Celsius` | Temperatura atual (ideal abaixo de 50 °C) |
...
# Conectar no servidor (cliente Minecraft)

-   **Java Edition (LAN):** `192.168.0.101:25565`

-   **Internet:** usar o link público gerado pelo playit.gg
...
**Monitores gráficos via Docker (App Store):**

| App | Acesso | Observação |
|---|---|---|
| **Netdata** | `http://IP:19999` | O mais completo, dashboard web em tempo real |
| **Glances** | Web UI | Leve, simples |
| **Cockpit** | Web UI | Gerenciamento de sistema mais completo |
